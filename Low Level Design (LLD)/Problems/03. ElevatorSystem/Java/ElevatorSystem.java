...
class ElevatorController {
    ...
    Set<Integer> processedRequests = new HashSet<>();
    Map<Integer, Integer> destinationFloors = new HashMap<>();

    public void addRequest(int floor, Direction direction, int destinationFloor) {
        if (processedRequests.contains(floor)) return;
    ...
    private void moveToFloor(int floor) {
        ...
        int destination = destinationFloors.get(floor);
        Direction newDirection = (destination - floor) > 0 ? Direction.UP : Direction.DOWN;
        addRequest(destination, newDirection, destination);
