# 💬 Instagram System Design — High-Level Design (HLD) with Data Estimations
...
## 📊 Estimated Data Metrics (Representative Assumptions)
...
- Avg reels uploaded/day: **~50–200M**
- Avg post media size (processed): **~2–20 MB** (image vs short video)
- Total daily media ingest: **~400TB — 3PB/day** (depends on video mix)

### ▶️ Consumption

- Avg watch/view time per user/day: **~30–60 minutes**
- Peak streams per second (global): **~200K — 1M**
- CDN egress/day: **~multiple PBs**

### 🔔 Events & Social Actions

- Likes/comments/saves/shares per user/day: **~100 actions**
- Events ingestion rate: **~200K — 1M events/sec**
...
|       Analytics & events | Event lake            | Kafka → ClickHouse / BigQuery |
...
## 🚀 Data Flow: Create Post → Appear in Feed
...
## 🧠 Recommendations, Explore & Search

**Recommendation pipeline**

- Event ingestion (views, likes, watch time) → feature store → candidate retrieval (ANN) → re-ranking (deep model + heuristics) → diversification & freshness filters
...
SLOs: feed load <300ms for cached pages, rebuffer rate <1–3%, moderation SLA for critical violations (minutes to hours depending on severity).
...
- **Event-driven architecture** with Kafka/PubSub for decoupling ingestion → ML → analytics
...
- Emphasize **feed generation** (fanout strategies), **media lifecycle** (ingest → transcode → CDN → cache), and **ML pipelines** for recommendations and moderation.
- Discuss **trade-offs** (precompute vs compute-on-read, storage tiers, moderation automation).
- State assumptions clearly when giving numbers; tailor the scale to interview constraints (e.g., 10× smaller/larger).
