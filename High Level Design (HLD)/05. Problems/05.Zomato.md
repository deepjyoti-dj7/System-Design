# 🎯 Zomato-like System Design — High-Level Design (HLD) with Data Estimations

Below is a high-level architecture and quantitative sizing for a food-delivery & restaurant-discovery platform similar to **Zomato**.

> **Assumptions (baseline)**

- Active users: **100M** (monthly active users)
- Peak concurrent users: **2%** of active → **2.0M** concurrent
- Restaurants listed: **2M**
- Avg photos per restaurant: **20**
- Avg photo size: **0.5 MB**
- Avg orders per active user per month: **6** → **0.2 orders/user/day**
- Delivery partners (drivers): **4M** (registered, with subset active)
- Search queries per user/day: **5**
- Avg order payload/events: small JSON ≈ 2 KB
...
# 📊 Estimated Data Metrics & Calculations
...
- **Orders/user/day** = 0.2
  → **Daily orders** = 100,000,000 × 0.2 = **20,000,000 orders/day**

- Average seconds per day = 86,400
  → **Avg orders/s** = 20,000,000 / 86,400 ≈ **231.48 orders/s**

- Assume peak ≈ 10× average (morning/lunch/dinner spikes)
  → **Peak orders/s** ≈ **2,315 orders/s**

- Average order event size ≈ 2 KB (order created)
  → **Peak ingress bandwidth for orders** ≈ 2,315 × 2 KB/s ≈ **4.63 MB/s** (~37 Mbps) — small, but many supporting flows (images, maps, telemetry) add up.

### 2) Users & Sessions

- Peak concurrent users = 2% × 100M = **2,000,000 concurrent**
...
- **Avg photo size** = 0.5 MB
  → **Total photos** = 2,000,000 × 20 = 40,000,000 photos
  → **Storage** = 40,000,000 × 0.5 MB = 20,000,000 MB = **~20,000 GB = 20 TB**

- Add metadata, menus, icons — small relative to photos (couple TB).

### 4) Search & Queries

- **Search queries/user/day** = 5
  → **Total search queries/day** = 100,000,000 × 5 = **500,000,000/day**
  → Avg search QPS = 500M / 86,400 ≈ **5,787 qps**; peak maybe 3× → **~17.4k qps**.
  Requires scaled Elasticsearch clusters with caching (Redis) and proper sharding.

### 5) Telemetry (Delivery Partner Location)

- Active delivery partners (concurrent reporting) say 10% of drivers report location frequently: 4M × 10% = 400k active reporters; each reports every 10s
  → updates/s = 400,000 / 10 = **40,000 location updates/s**
...
- Avg order DB row size (including indexes) ≈ 2 KB
  → **Daily orders DB write** = 20M × 2 KB = 40,000,000 KB = ~40 GB/day
  → Yearly = 40 GB × 365 ≈ **14.6 TB/year** (raw; backups and indexes increase needs).
...
# 🚦 Order Flow (Sequence - simplified)
...
# 🛰️ Delivery Optimization & Routing
...
|                               Item |              Tool |                 TTL/Notes |
| ---------------------------------: | ----------------: | ------------------------: |
|          Restaurant metadata (hot) |             Redis |                 5–30 mins |
|                         Menu items |             Redis | 10–60 mins (menus change) |
|           Search results (popular) |             Redis |                 30–60 sec |
...
|        Business analytics | Kafka → S3 → Redshift/BigQuery |
...
# 🧾 Example Cost/Capacity Notes (rough)

- **S3 for images**: 20 TB storage (from earlier) — modest cost compared to traffic.
- **DB storage**: Orders / indices → tens of TB/year as orders accumulate.
...
- These numbers are **estimates** based on assumptions listed at top. If you want:
  ...
  - or a capacity plan with instance counts & cost estimate — I can produce those next.
