# 💬 YouTube System Design — High-Level Design (HLD) with Data Estimations
...
## 📊 Estimated Data Metrics (Example Assumptions)

> **Scale assumptions** — pick numbers you want to defend in an interview; these are representative of a very large service.

### 👥 User Base

- Monthly Active Users: **~2.5B**
- Daily Active Users (DAU): **~1B**
- Peak concurrent viewers (all regions): **~50–100M+** (during global events)

### ▶️ Content & Uploads

- Avg videos uploaded / day: **~500K — 2M** (varies by assumption)
- Avg video length: **8–12 minutes**
- Avg stored video size (after renditions): **~300–800 MB** (depends on encoding & length)
- Total new video bytes/day (ingest, raw + renditions): **~100–500 TB/day**

### 📺 Playback & CDN

- Peak video streams/sec: **~500K — 2M** (global)
- Avg watch time per user/day: **~30–60 minutes**
- CDN cache hit ratio: **~85–95%** for popular segments
...
- Events ingestion rate: **~100K — 1M events/sec**
...
## 🛰️ Content Delivery & Playback Optimizations

- **ABR (Adaptive Bitrate)**: Provide multiple renditions (240p → 2160p+) with segment durations tuned for latency.
- **Segmented caching**: Cache small segments (2–6s) for fast quality shifts.
...
## 🧠 Recommendations, Search & ML

**Recommendation pipeline**

- Real-time event stream → feature generation → candidate retrieval (approximate nearest neighbors) → re-ranking (DNN + business rules) → renderer (diversified feed)
...
| -------------------------: | ------------------------- | ---------------------------------- |
|   Popular video renditions | CDN                       | Minutes → hours (LRU + popularity) |
...
- **Event streaming (Kafka/PubSub)** for decoupling ingestion → processing → analytics
...
## 📊 Final Notes (what to emphasize in an interview)

- Focus on **data flow** (ingest → transcode → storage → CDN → client) and how you handle **scale** at each stage.
...
- Always state assumptions when giving numbers — interviewers appreciate defensible estimates.
